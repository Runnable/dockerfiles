...
export WORDPRESS_DB_HOST WORDPRESS_DB_USER WORDPRESS_DB_PASSWORD MYSQL_ROOT_PASSWORD WORDPRESS_DB_NAME
...
if [ "$WORDPRESS_DB_NAME" ]; then
  ...
  mysql+=( "$WORDPRESS_DB_NAME" )
fi

if [ "$WORDPRESS_DB_USER" -a "$WORDPRESS_DB_PASSWORD" ]; then
  echo "Creating user: $WORDPRESS_DB_USER"
  echo "CREATE USER '$WORDPRESS_DB_USER'@'%' IDENTIFIED BY '$WORDPRESS_DB_PASSWORD' ;" | "${mysql[@]}"

  if [ "$WORDPRESS_DB_NAME" ]; then
    echo "GRANT ALL ON \`$WORDPRESS_DB_NAME\`.* TO '$WORDPRESS_DB_USER'@'%' ;" | "${mysql[@]}"
  fi
...
echo
echo 'Mysql init process done. Setting up Wordpress'
echo
